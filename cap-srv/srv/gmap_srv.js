...
module.exports = class GmapsService extends cds.ApplicationService {
    ...
    async init() {
        ...
        this.on('getDirections', async (req) => {
            ...
            // Resolve SAP sandbox codes to real addresses for demo
            const addressMap = {
            ...
            const resolvedFrom = addressMap[from] || from;
            const resolvedTo   = addressMap[to]   || to;

            try {
                console.log(`Fetching directions from "${resolvedFrom}" to "${resolvedTo}"...`);

                const queryParams = new URLSearchParams({ origin: resolvedFrom, destination: resolvedTo });
                ...
                if (response.status !== 'OK') {
                    ...
                    console.error(`Google Maps API returned status "${response.status}" for route "${resolvedFrom}" → "${resolvedTo}": ${errorMsg}`);
                    return req.error(400, `Google Maps API error (status: ${response.status}) for route "${resolvedFrom}" to "${resolvedTo}": ${errorMsg}`);
                }
